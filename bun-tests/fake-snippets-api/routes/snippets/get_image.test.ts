...
test("get image of schematic in SVG format", async () => {
  const { axios, db } = await getTestServer()
  ...
  const response = await axios.get(
    "/api/snippets/get_image?snippet_id=test_snippet_id&image_of=schematic&format=svg",
  )
...
test("get image of schematic in SVG format using alias", async () => {
  const { axios, db } = await getTestServer()
  ...
  const response = await axios.get(
    "/snippets/images/testuser/TestSnippet/schematic.svg",
  )
